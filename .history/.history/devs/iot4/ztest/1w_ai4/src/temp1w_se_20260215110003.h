...
public:
  ...
  void publishCurrentState() override {
    // Publish current state of all sensors when MQTT connects
    if (_mqtt == nullptr || !_mqtt->isConnected()) return;
    
    for (uint8_t i = 0; i < TW1_SENSOR_COUNT; i++) {
      float tempF = t1w_state[t1w_config[i].sa].lastValue;
      
      // Only publish if we have a valid reading (non-zero)
      if (tempF != 0.0) {
        ...
        Serial.printf("Published initial: %s -> %s\n", topic.c_str(), payload.c_str());
      }
    }
  }
